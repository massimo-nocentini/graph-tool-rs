...
1. **File ownership is disjoint and exhaustive.** A unit edits only the files
   ...
   own, that is a design error: stop and raise it against `docs/DESIGN.md`
...
5. **A unit is not done because it compiles.** §16 of DESIGN.md exists because
...
* **trybuild compile-fail fixtures**, each asserting the exact diagnostic:
  ...
  - `&dyn GraphRef` → compile error (the trait is intentionally not a trait
    object; `DynGraph` is).
  These four are quoted in DESIGN.md §14 and must not be allowed to regress.
...
boundary is O(in-degree) and was measured quadratic; see DESIGN.md §3.
...
* `size_of` assertions for all six views (DESIGN.md §11).
...
* Codegen assertion: a scan through `PropSlice::as_slice()` has no
  `panic_bounds_check`; a per-key scatter does (documented cost, DESIGN.md
  §12).
...
* `chunked_sum::<4>` matches a sequential Kahan sum to within the documented
  ...
  the test must *record* the answer, not assume it (DESIGN.md §12).
...
* **`pair_mut(xs, 3, 1)` returns `Two(&mut xs[3], &mut xs[1])` — in that
  ...
  *value*, not the index. A sorted return silently transposes; see DESIGN.md
  D9.
...
(`fast_vector_property_map.hh:129`); see DESIGN.md §5.
...
if it does, that is a finding against DESIGN.md §10 and must be raised.
...
* Instantiation count assertion: `nm` on a release build shows **45**
  conversion monomorphisations for the full lattice, not 225 (DESIGN.md §10).
...
These are tracked in DESIGN.md §15 and must not be started as part of a unit
